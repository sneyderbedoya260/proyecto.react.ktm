...
export async function crearProducto(req, res) {
  ...
  const [resultado] = await pool.query('INSERT INTO productos (titulo, descripcion, detalle, categoria, imagen_url, precio) VALUES (?, ?, ?, ?, ?, ?)', [titulo, descripcion, detalle, categoria, imagen_url, precio || 0]);
...
export async function actualizarProducto(req, res) {
  ...
  const [resultado] = await pool.query('UPDATE productos SET titulo=?, descripcion=?, detalle=?, categoria=?, imagen_url=?, precio=?, estado=? WHERE id=?', [titulo, descripcion, detalle, categoria, imagen_url, precio, estado, req.params.id]);
