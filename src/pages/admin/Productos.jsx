...
const formularioVacio = {
  titulo: '', descripcion: '', detalle: '', categoria: '', imagen_url: '', precio: '', estado: 'Disponible',
};
...
function AdminProductos() {
  ...
  const manejarArchivoSeleccionado = async (e) => {
    ...
    try {
      ...
      setFormulario((a) => ({ ...a, imagen_url: resultado.url }));
    } catch (error) {
  ...
  const iniciarEdicion = (producto) => {
    ...
    setFormulario({
      titulo: producto.titulo || '', descripcion: producto.descripcion || '', detalle: producto.detalle || '',
      categoria: producto.categoria || '', imagen_url: producto.imagen_url || '', precio: producto.precio || '',
  ...
  return (
    <div className="mx-auto max-w-5xl text-white">
      ...
      <p className="mt-2 text-neutral-300">Agrega, edita o elimina los modelos que aparecen en el catálogo.</p>
      ...
      <form onSubmit={enviarFormulario} className="mt-6 grid gap-4 rounded-lg border-t-4 border-[var(--ktm-orange)] bg-[var(--ktm-gray)] p-6 sm:grid-cols-2">
        ...
        <div className="grid gap-2 text-sm sm:col-span-2">
          <label className="grid gap-1">Imagen del producto
            <input type="file" accept="image/*" className="rounded border border-neutral-600 bg-black/40 px-3 py-2 text-neutral-200 file:mr-3 file:rounded file:border-0 file:bg-[var(--ktm-orange)] file:px-3 file:py-2 file:font-bold file:text-black" onChange={manejarArchivoSeleccionado} disabled={subiendoImagen} />
          </label>
          {subiendoImagen && <span className="text-xs text-neutral-400">Subiendo imagen...</span>}
          {formulario.imagen_url && !subiendoImagen && <img src={formulario.imagen_url} alt="Vista previa" className="h-24 w-36 rounded object-cover border border-neutral-700" />}
          <label className="grid gap-1">O pega una URL de imagen
            <input className="rounded border border-neutral-600 bg-black/40 px-3 py-2" name="imagen_url" value={formulario.imagen_url} onChange={actualizarCampo} placeholder="https://.../moto.jpg" />
          </label>
        </div>
          ...
          <table className="w-full min-w-[640px] border-collapse text-left text-sm">
            <thead>
              <tr className="bg-[var(--ktm-gray)] text-neutral-300">
                <th className="p-3">Foto</th><th className="p-3">Título</th><th className="p-3">Categoría</th><th className="p-3">Precio</th><th className="p-3">Estado</th><th className="p-3">Acciones</th>
              </tr>
            </thead>
            <tbody>
              {productos.map((p) => (
              ...
              {productos.length === 0 && <tr><td className="p-4 text-center text-neutral-400" colSpan={6}>No hay productos todavía.</td></tr>}
            </tbody>
          </table>
        </div>
      )}
